...
export default function BulkWithdrawalModal({ isOpen, onClose }: BulkWithdrawalModalProps) {
  const { clients, generateWithdrawalId, bulkAddWithdrawals, setWithdrawals } = useClientContext()
  ...
  const [previewData, setPreviewData] = useState<Withdrawal[]>([])
  ...
  const validateAndPreview = (data: string) => {
    ...
    try {
      ...
      for (const line of previewLines) {
        ...
        // Create withdrawal object for preview
        const withdrawal: Withdrawal = {
          withdrawalId: `${generateWithdrawalId()}_preview_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`,
          shopId,
          clientName,
          agent,
          date: parsedDate,
        ...
        previewWithdrawals.push(withdrawal)
      }

      setPreviewData(previewWithdrawals)

      if (previewErrors > 0 && previewWithdrawals.length === 0) {
    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        if (lines[i].trim()) {
          ...
          const withdrawal: Withdrawal = {
            withdrawalId: `${generateWithdrawalId()}_${i}_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`,
            shopId,
            clientName,
            agent,
            date: parsedDate,
      ...
      if (allWithdrawals.length > 0) {
        try {
          ...
          await bulkAddWithdrawals(allWithdrawals)
          ...
          setError(`Error saving withdrawals: ${error.message}`)

          toast({
            variant: "destructive",
            title: "Import Failed",
            description: `There was an error saving the withdrawals. Some data may not have been saved.`,
          })
        }
      } else {
  ...
  const downloadSampleCsv = () => {
    ...
    a.href = url
    a.download = "withdrawal_sample.csv"
            ...
            {previewData.length > 0 ? (
              <div className="rounded-md border overflow-hidden">
                <Table>
                  ...
                  <TableBody>
                    {previewData.map((withdrawal, index) => (
                      <TableRow key={index}>
